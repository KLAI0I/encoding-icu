...
<head>
  ...
  <title>ICU Dashboard Bridge</title>
</head>
<body style="margin:0;font-family:system-ui,Segoe UI,Arial">
<script>
(function(){
  // Whitelist is optional; keep it permissive for your internal tool.
  function post(type, payload){
    try{
      parent.postMessage(Object.assign({__icu_bridge: true, type: type}, payload||{}), "*");
    }catch(e){}
  }

  window.addEventListener("message", function(ev){
    const msg = ev && ev.data;
    if(!msg || msg.__icu_bridge_call !== true) return;

    const id = msg.id;
    const fn = String(msg.fn || "");
    const args = Array.isArray(msg.args) ? msg.args : [];

    if(!fn || !google || !google.script || !google.script.run || typeof google.script.run[fn] !== "function"){
      post("result", { id: id, ok: false, error: "Unknown server function: " + fn });
      return;
    }
    ...
    try{
      google.script.run
        .withSuccessHandler(function(result){
          post("result", { id: id, ok: true, result: result });
        })
        .withFailureHandler(function(err){
          post("result", { id: id, ok: false, error: (err && err.message) ? err.message : String(err) });
        })[fn].apply(google.script.run, args);
    }catch(e){
      post("result", { id: id, ok: false, error: (e && e.message) ? e.message : String(e) });
    }
  });
...
function doGet(e) {
  ...
  // Normal Apps Script-hosted UI (optional; does not affect GitHub hosting)
